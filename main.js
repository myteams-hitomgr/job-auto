...
async function downloadAndPrepareCSV(browser, acc) {
  ...
  try {
    ...
    while (true) {
      ...
      // 🎯 【重要修正】実態に合わせ、ステータスに「完了」か「成功」が入っていれば100%抽出成功と判定して次へ進む
      if (cleanStatus.includes('完了') || cleanStatus.includes('成功')) {
        console.log(`✅ 【${acc.name}】最新の取出行でCSVの生成完了を確認しました！`);
        break;
      }
      
      // 現在の状態（待機中、進行中など）をコンソールに表示
    ...
    if (finalLatestRow && await finalLatestRow.count() > 0) {
    ...
    if (!downloadLink || await downloadLink.count() === 0) {
      throw new Error("CSVのダウンロードリンクが特定できませんでした。");
    }
...
(async () => {
  ...
  console.log(`🤖 起動回数: ${counterData.count + 1}回目 → 今回の処理: 【${currentState}】`);
